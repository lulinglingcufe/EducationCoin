...
const logger = require('../../../src/comm/util').getLogger('token-network-transfer.js');
const os = require('os');
...
module.exports.init = async function(blockchain, context, args) {
    ...
    try {
		...
		for (let i=0; i<(testAssetNum*2 +2); i++) {
			let participant = factory.newResource(namespace, 'User', 'user7@QQ.com'  + '_' + i);
...
module.exports.run = function() {
	...
	--testAssetNum;
    transaction.fromuser = factory.newRelationship(namespace, 'User', 'user7@QQ.com'  + '_' + (testAssetNum*2 + 1) );
	transaction.to = factory.newRelationship(namespace, 'User', 'user7@QQ.com' + '_' +  (testAssetNum* 2) );
