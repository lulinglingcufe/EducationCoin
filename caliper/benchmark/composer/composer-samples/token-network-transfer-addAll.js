...
const logger = require('../../../src/comm/util').getLogger('token-network-transfer-addAll.js');
const os = require('os');
...
module.exports.init = async function(blockchain, context, args) {
    ...
    try {
		...
		for (let i=0; i<(testAssetNum*2 +2); i++) {
			let participant = factory.newResource(namespace, 'User', 'user8@QQ.com'  + '_' + i);
			participant.accountBalance = 1000;
            participants.push(participant);
			//await participantRegistry.add(participant);
        }
		await participantRegistry.addAll(participants);
		
		let exists = await participantRegistry.exists('regulator@qq.com' + uuid);
        if (exists) {
...
module.exports.run = function() {
	...
	--testAssetNum;
    transaction.fromuser = factory.newRelationship(namespace, 'User', 'user8@QQ.com'  + '_' + (testAssetNum*2 + 1) );
	transaction.to = factory.newRelationship(namespace, 'User', 'user8@QQ.com' + '_' +  (testAssetNum* 2) );
	
	transaction.transferNum = 2;
