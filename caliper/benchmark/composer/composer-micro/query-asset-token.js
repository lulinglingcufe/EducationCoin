...
module.exports.init = async function(blockchain, context, args) {
	...
	let participant = factory.newResource(namespace, 'User', 'User10@qq.com');
    participant.accountBalance = 100;
    await participantRegistry.add(participant);
	
	
    for(let i = 0; i < testAssetNum; i ++){
        let userTokenRecharge = factory.newResource(namespace, 'UserTokenRecharge', 'REcharge10_' + i);
        
		userTokenRecharge.user = factory.newRelationship(namespace, 'User', 'User10@qq.com');
...
module.exports.run = function() {
    ...
    return busNetConnection.query('GetUserTokenRechargeUserR', { user: 'resource:token.User#User10@email.com'})
        .then((result) => {
